...
const getClassNamesFromNodes = nodes =>
  nodes.map(node => node.data.class.split(" ")[0]);
...
const getPrettyHtmlFromNode = node => {
  ...
  const htmlWithRemovedServerRenderedText = html.replace(
...
module.exports = {
  ...
  print(received) {
    ...
    const prettyStyles = getPrettyStylesFromClassNames(
      classNames,
      styleElements
    );
